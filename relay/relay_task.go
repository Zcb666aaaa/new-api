...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	baseQuotaBeforeRatios := info.PriceData.Quota
	...
	// 计费详情日志
	{
		otherRatiosJSON, _ := common.Marshal(info.PriceData.OtherRatios)
		logger.LogInfo(c, fmt.Sprintf(
			"[TaskBilling] model=%s channel_id=%d | baseQuota=%d otherRatios=%s finalQuota=%d | ModelPrice=%.6f ModelRatio=%.6f GroupRatio=%.6f FreeModel=%v UsePrice=%v",
			modelName, info.ChannelId,
			baseQuotaBeforeRatios, string(otherRatiosJSON), info.PriceData.Quota,
			info.PriceData.ModelPrice, info.PriceData.ModelRatio,
			info.PriceData.GroupRatioInfo.GroupRatio,
			info.PriceData.FreeModel, info.PriceData.UsePrice,
		))
	}
	...
	if requestBody != nil {
		...
		if !strings.HasPrefix(contentType, "multipart/form-data") {
			...
			if overridden, overrideErr := relaycommon.ApplyParamOverrideWithRelayInfo(bodyForOverride, info); overrideErr == nil {
				...
				logger.LogError(c, fmt.Sprintf("[TaskRelay] 参数覆盖应用失败 channel_id=%d, err=%v", info.ChannelId, overrideErr))
			}
		}
		...
		contentType = c.Request.Header.Get("Content-Type")
		if strings.HasPrefix(contentType, "multipart/form-data") && c.Request.PostForm != nil {
	...
	if resp != nil {
		channelName := adaptor.GetChannelName()
		requestURL, _ := adaptor.BuildRequestURL(info)
		common.SysLog(fmt.Sprintf("[%s] 渠道请求成功 - URL: %s, StatusCode: %d", channelName, requestURL, resp.StatusCode))
	}
