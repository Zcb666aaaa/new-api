...
func (a *TaskAdaptor) BuildRequestURL(info *relaycommon.RelayInfo) (string, error) {
	return fmt.Sprintf("%s/video-synthesis", a.baseURL), nil
}
...
func (a *TaskAdaptor) BuildRequestBody(c *gin.Context, info *relaycommon.RelayInfo) (io.Reader, error) {
	...
	// 创建用于日志打印的副本，截断图片字段
	aliReqForLog := *aliReq
	if len(aliReqForLog.Input.ImgURL) > 100 {
		aliReqForLog.Input.ImgURL = aliReqForLog.Input.ImgURL[:100] + "..."
	}
	if len(aliReqForLog.Input.FirstFrameURL) > 100 {
		aliReqForLog.Input.FirstFrameURL = aliReqForLog.Input.FirstFrameURL[:100] + "..."
	}
	if len(aliReqForLog.Input.LastFrameURL) > 100 {
		aliReqForLog.Input.LastFrameURL = aliReqForLog.Input.LastFrameURL[:100] + "..."
	}
...
func (a *TaskAdaptor) convertToAliRequest(info *relaycommon.RelayInfo, req relaycommon.TaskSubmitReq) (*AliVideoRequest, error) {
	...
	if req.Metadata != nil {
		...
		if metadataBytes, err := common.Marshal(req.Metadata); err == nil {
			err = common.Unmarshal(metadataBytes, &metadata)
			if err != nil {
