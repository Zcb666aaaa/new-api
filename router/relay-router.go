...
func SetRelayRouter(router *gin.Engine) {
	...
	}
	{
		...
		httpRouter.POST("/images/generations", func(c *gin.Context) {
			// 检查是否为算能渠道
			if channelType, exists := c.Get("channel_type"); exists && channelType == constant.ChannelTypeSophnet {
	...
	relaySophnetRouter := router.Group("/sophnet")
	relaySophnetRouter.Use(middleware.RouteTag("relay"))
	relaySophnetRouter.Use(middleware.SystemPerformanceCheck())
	relaySophnetRouter.Use(middleware.TokenAuth(), middleware.Distribute())
	{
		relaySophnetRouter.POST("/submit/:action", controller.RelayTask)
		relaySophnetRouter.POST("/fetch", controller.RelayTaskFetch)
		relaySophnetRouter.GET("/fetch/:id", controller.RelayTaskFetch)
	}

	// 算能云异步图片生成路由
	relayImageGeneratorRouter := router.Group("/imagegenerator")
	relayImageGeneratorRouter.Use(middleware.RouteTag("relay"))
	relayImageGeneratorRouter.Use(middleware.SystemPerformanceCheck())
	relayImageGeneratorRouter.Use(middleware.TokenAuth(), middleware.Distribute())
	{
		relayImageGeneratorRouter.POST("/task", controller.RelayTask)
		relayImageGeneratorRouter.GET("/task/:id", controller.RelayTaskFetch)
	}

	// 算能云异步视频生成路由
	relayVideoGeneratorRouter := router.Group("/videogenerator")
	relayVideoGeneratorRouter.Use(middleware.RouteTag("relay"))
	relayVideoGeneratorRouter.Use(middleware.SystemPerformanceCheck())
	relayVideoGeneratorRouter.Use(middleware.TokenAuth(), middleware.Distribute())
	{
		relayVideoGeneratorRouter.POST("/generate", controller.RelayTask)
		relayVideoGeneratorRouter.GET("/generate/:id", controller.RelayTaskFetch)
	}
