...
var defaultModelPricePerSecond = map[string]float64{
	// 按秒计费的模型（固定价格按秒计算）
}
...
func InitRatioSettings() {
	...
	modelPricePerSecondMap.AddAll(defaultModelPricePerSecond)
...
func ModelPricePerSecond2JSONString() string {
	return modelPricePerSecondMap.MarshalJSONString()
}

func UpdateModelPricePerSecondByJSONString(jsonStr string) error {
	return types.LoadFromJsonStringWithCallback(modelPricePerSecondMap, jsonStr, InvalidateExposedDataCache)
}

// GetModelPricePerSecond 返回按秒计费的模型价格，如果不存在则返回 -1, false
func GetModelPricePerSecond(name string, printErr bool) (float64, bool) {
	name = FormatMatchingModelName(name)

	if strings.HasSuffix(name, CompactModelSuffix) {
		price, ok := modelPricePerSecondMap.Get(CompactWildcardModelKey)
		if !ok {
			if printErr {
				common.SysError("model price per second not found: " + name)
			}
			return -1, false
		}
		return price, true
	...
	price, ok := modelPricePerSecondMap.Get(name)
	if !ok {
		if printErr {
			common.SysError("model price per second not found: " + name)
		}
...
func IsModelPricePerSecond(name string) bool {
	_, ok := GetModelPricePerSecond(name, false)
	return ok
}
...
func GetModelPrice(name string, printErr bool) (float64, bool) {
	...
	// fallback to per-second price if defined
	price, ok = modelPricePerSecondMap.Get(name)
	if !ok {
		if printErr {
			common.SysError("model price not found: " + name)
