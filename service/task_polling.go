...
func DispatchPlatformUpdate(platform constant.TaskPlatform, taskChannelM map[int][]string, taskM map[string]*model.Task) {
	...
	case constant.TaskPlatformSophnet:
...
func updateVideoSingleTask(ctx context.Context, adaptor TaskPollingAdaptor, ch *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	if err != nil {
		// FetchTask 失败，将任务标记为失败状态
		logger.LogError(ctx, fmt.Sprintf("FetchTask failed for task %s: %v", taskId, err))
		snap := task.Snapshot()
		task.Status = model.TaskStatusFailure
		task.Progress = taskcommon.ProgressComplete
		task.FailReason = fmt.Sprintf("fetch task failed: %v", err)
		if task.FinishTime == 0 {
			task.FinishTime = time.Now().Unix()
		}
		if won, updateErr := task.UpdateWithStatus(snap.Status); updateErr != nil {
			logger.LogError(ctx, fmt.Sprintf("UpdateWithStatus failed for task %s: %s", task.TaskID, updateErr.Error()))
	...
	case model.TaskStatusInProgress:
		...
		if task.StartTime == 0 {
			// 优先使用 adaptor 返回的开始时间
			if taskResult.StartTime > 0 {
				task.StartTime = taskResult.StartTime
			} else {
				task.StartTime = now
			}
		}
	case model.TaskStatusSuccess:
		task.Progress = taskcommon.ProgressComplete
		// 优先使用 adaptor 返回的完成时间
		if task.FinishTime == 0 {
			if taskResult.FinishTime > 0 {
				task.FinishTime = taskResult.FinishTime
			} else {
				task.FinishTime = now
			}
		}
		// 如果有 adaptor 返回的开始时间且本地尚未设置，也更新开始时间
		if task.StartTime == 0 && taskResult.StartTime > 0 {
			task.StartTime = taskResult.StartTime
		}
