...
func LogTaskConsumption(c *gin.Context, info *relaycommon.RelayInfo) {
	...
	// 任务计费日志需要关联 task_id，以便后续差额结算更新同一条日志，而不是新增一条。
...
// isAsyncTaskChannel 判断渠道类型是否为需要日志合并的异步任务渠道。
func isAsyncTaskChannel(channelType int) bool {
	switch channelType {
	case constant.ChannelTypeSophnet,
...
func mergeAsyncTaskBillingLog(task *model.Task, deltaQuota int, other map[string]interface{}) bool {
	...
	if err != nil || ch == nil || !isAsyncTaskChannel(ch.Type) {
		return false
	}
	// 需要 task_id 关联
	if task.TaskID == "" {
	...
	// 查找最近一条消费日志（type=consume）中包含 task_id 的记录
	likePattern := fmt.Sprintf("%%\"task_id\":\"%s\"%%", task.TaskID)
	var log model.Log
	if err := model.LOG_DB.Where("user_id = ? AND type = ? AND other LIKE ?", task.UserId, model.LogTypeConsume, likePattern).
	...
	log.Other = common.MapToJsonStr(existingOther)
